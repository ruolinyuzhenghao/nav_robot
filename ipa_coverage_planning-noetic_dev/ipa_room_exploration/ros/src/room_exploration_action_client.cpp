...
int main(int argc, char **argv)
{
	...
	double robot_radius;
	priv_nh.param("robot_radius", robot_radius, 100.5);
	double coverage_radius;
	priv_nh.param("coverage_radius", coverage_radius, 2.0);
	...
	else
	{
		...
		priv_nh.param<std::string>("env_pack", env_pack_path);
		std::string file_name;
		priv_nh.param<std::string>("image", file_name, "map.pgm");

		image_path = env_pack_path + "/" + file_name;
		std::cout<<image_path;
	}
...
//	int planning_mode = 1;	// footprint planning
	...
	std::string txt_path = "/home/rover/coverage_path.txt"; 
	...
	for (size_t point=0; point<action_result->coverage_path.size(); ++point)
	{
		...
		//保存路径点到path.txt文件
		output_stream<< action_result->coverage_path[point].x;
		output_stream<<"  ";
		output_stream<<action_result->coverage_path[point].y;
	...
	cv::imwrite("/home/rover/path.jpg",path_map);
	cv::waitKey();
